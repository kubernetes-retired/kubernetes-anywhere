...
class Config(object):
    ...
    def write_config(self, filename, header=None, CONFIG_='CONFIG_'):
...
class Symbol(Item):
    ...
    def _make_conf(self, append_fn):
        ...
        prefix = os.environ.get('CONFIG_')
        if prefix is None:
            prefix = 'CONFIG_'

        if self.type == BOOL or self.type == TRISTATE:
            append_fn("{0}{1}={2}".format(prefix, self.name, val)
                      if val == "y" or val == "m" else
                      "# {0}{1} is not set".format(prefix, self.name))
