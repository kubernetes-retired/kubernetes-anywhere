...
You will have to run the commands below agains a local Docker (via UNIX socket), remote HTTP connectivity is not supported.

You must use Docker Engine not older then v1.10.

A separate Docker Machine example is also provided.
...
docker run --rm -ti \
  ...
  weaveworks/kubernetes-anywhere:toolbox sh -c 'setup-kubelet-volumes && compose -p kube pull && compose -p kube up -d'
```
