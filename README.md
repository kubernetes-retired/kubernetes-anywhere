...
> without the need to set it all up from scratch. Deploy Weave Net first, then `docker run` cluster
...
are very easy, as everything is containerized.

In a nutshell, each of the cluster components gets a fixed DNS names, e.g. `kube-apiserver.weave.local`,
