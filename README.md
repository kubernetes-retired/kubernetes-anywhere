...
> ## TL;DR
> 
> Make Kubernetes cluster setup super easy with Weave Net. Pick your tools and your cloud,
> but no need to set it all up from scratch. Several provisioning examples provided.
...
to any infrastructure. By implementing Weave Net as a cluster management/bootstrap network,
complete portability is ensured, allowing you to move or clone an entire cluster. Even provisioning
of security certificates is fully transparent, and, needless to say, in-place cluster upgrades
are very easy, as everything is containerised.

Because Weave Net manages its state without [the use of a externaly store][bryans-talk], you
...
In the nutshell, each of the cluster components gets a fixed DNS names, e.g. `kube-apiserver.weave.local`,
`etcd1.weave.local` ...etc. Having this enables you to focus on higher level problems, i.e. make
cluster boostrap the responsibility of the network and put less wieght on already complex provisioning
...
We are currently shipping descrete containers (no self-hosting). You will find up-to-date images on Docker
...
directory, if you with to know what goes in exactly.

Images are tagged with component name and version. If you pull `weaveworks/kubernetes-anywhere:apiserver-v1.2`,
you will get the latest build of `v1.2.x`, so you upgrading your cluster to patch releases is very easy.
However, if you don't like this, there are tags for patch releases also. We may update any of the versioned
image without changing the version of Kubernetes base image, but you can use Docker's new content hashes,
if you feel uncertain/paranoid.

![Travis CI](https://img.shields.io/travis/weaveworks/weave-kubernetes-anywhere.svg?style=flat-square)
...
## Full-blow Cluster Example (Using Terraform in EC2)
...
Once instances are up, you can login to either of the 3 worker nodes or the master and run 
...
## Single Docker Host Example (Using Docker Compose)
...
## Containerised Provisioning of Certificates - way easier then ever!
